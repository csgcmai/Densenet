...
def get_symbol(
    ...
    conv = mx.symbol.Pooling(conv, name='conv0_pool', global_pool=False, kernel=(3,3), stride=(2,2), pad=(1,1), pool_type ='max')
    ...
    conv = mx.symbol.Pooling(conv, global_pool=True, kernel=(16, 2), pool_type='avg', name = 'global_avg_pool')
    flat = mx.symbol.Flatten(data=conv)
    fc = mx.symbol.FullyConnected(data=flat, num_hidden=num_class, name='fc')

    return mx.symbol.SoftmaxOutput(data=fc, name='softmax'), flat
